...
@implementation YQHTextAttachment
//I want my emoticon has the same size with line's height
- (CGRect)attachmentBoundsForTextContainer:(NSTextContainer *)textContainer proposedLineFragment:(CGRect)lineFrag glyphPosition:(CGPoint)position characterIndex:(NSUInteger)charIndex NS_AVAILABLE_IOS(7_0)
{
    return CGRectMake( 0, -3.0f, lineFrag.size.height, lineFrag.size.height);
}

@end


@interface YQHEmotionEscape ()
{
...
@implementation YQHEmotionEscape

static YQHEmotionEscape *_sharedInstance = nil;
...
+ (YQHEmotionEscape *)sharedInstance
{
    ...
    {
        @synchronized(self) {
            _sharedInstance = [[YQHEmotionEscape alloc] init];
        }
    }
...
- (NSMutableAttributedString *) attributtedStringFromText:(NSString *) aInputText
{
    ...
    for (NSTextCheckingResult *match in [matches reverseObjectEnumerator]) {
        ...
        YQHTextAttachment * textAttachment = [[YQHTextAttachment alloc ] initWithData:nil ofType:nil];
...
- (NSAttributedString *) attStringFromTextForChatting:(NSString *) aInputText textFont:(UIFont*)font
{
    ...
    if (font) {
        ...
        //[string addAttribute: value:<#(nonnull id)#> range:<#(NSRange)#>];
    }
    return string;
}

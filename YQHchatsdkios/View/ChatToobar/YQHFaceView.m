...
@property (nonatomic, strong) YQHFacialView *facialView;

@end

@implementation YQHFaceView
...
- (YQHFacialView*)facialView
{
    if (_facialView == nil) {
        _facialView = [[YQHFacialView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 150)];
...
- (void)_setupButtonScrollView
{
    ...
    for (int i = 0; i < number; i++) {
        ...
        YQHEmotionManager *emotionManager = [_emotionManagers objectAtIndex:i];
        if (emotionManager.emotionType == YQHEmotionDefault) {
            YQHEmotion *emotion = [emotionManager.emotions objectAtIndex:0];
...
- (void)setEmotionManagers:(NSArray *)emotionManagers
{
    ...
    for (YQHEmotionManager *emotionManager in _emotionManagers) {
        if (emotionManager.emotionType != YQHEmotionGif) {
